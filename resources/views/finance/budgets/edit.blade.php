...
<div class="container py-4">
    ...
    <form action="{{ route('finance.budgets.update', $budget->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <input type="number" name="year" id="year" class="form-control" value="{{ old('year', $budget->year) }}"
                required>
            @error('year')
            <small class="text-danger">{{ $message }}</small>
            @enderror
        ...
        <div class="mb-3">
            ...
            <select name="status" id="status" class="form-select">
                ...
                <option value="Approved" {{ old('status', $budget->status) == 'Approved' ? 'selected' : '' }}>Approved
                </option>
                <option value="Rejected" {{ old('status', $budget->status) == 'Rejected' ? 'selected' : '' }}>Rejected
                </option>
            </select>
            @else
