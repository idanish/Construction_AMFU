...
<div class="container py-4">
    ...
    <form action="{{ route('finance.procurements.update', $procurement->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="item_name" id="item_name" class="form-control" value="{{ old('item_name', $procurement->item_name) }}" required>
            @error('item_name') 
        ...
        <div class="mb-3">
            ...
            <input type="number" name="quantity" id="quantity" class="form-control" value="{{ old('quantity', $procurement->quantity) }}" required>
            @error('quantity') 
                <small class="text-danger">{{ $message }}</small> 
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="number" name="cost_estimate" id="cost_estimate" class="form-control" step="0.01" value="{{ old('cost_estimate', $procurement->cost_estimate) }}" required>
            @error('cost_estimate') 
                <small class="text-danger">{{ $message }}</small> 
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <select name="department_id" id="department_id" class="form-select" required>
                ...
                @foreach($departments as $dept)
                    <option value="{{ $dept->id }}" {{ old('department_id', $procurement->department_id) == $dept->id ? 'selected' : '' }}>
                        {{ $dept->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <div id="filePreview" class="mt-2">
                @if($procurement->attachment)
                    📎 Current File: <a href="{{ asset('storage/' . $procurement->attachment) }}" target="_blank">{{ basename($procurement->attachment) }}</a>
                @endif
            </div>
            @error('attachment')
                <small class="text-danger">{{ $message }}</small>
